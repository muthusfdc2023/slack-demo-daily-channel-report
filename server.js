...
function verifySlackRequest(req) {
  try {
    ...
    if (!signingSecret) {
      console.warn('verifySlackRequest: SLACK_SIGNING_SECRET not set');
      return false;
    }
    ...
    if (!timestamp || !sig) {
      console.warn('verifySlackRequest: missing signature headers', { timestamp: !!timestamp, signature: !!sig });
      return false;
    }
    ...
    if (Number.isNaN(ts) || Math.abs(Math.floor(Date.now() / 1000) - ts) > fiveMinutes) {
      console.warn('verifySlackRequest: timestamp check failed', { ts });
      return false;
    }

    if (!req.rawBody) {
      console.warn('verifySlackRequest: rawBody missing');
      return false;
    }

    const basestring = `v0:${timestamp}:${req.rawBody.toString()}`;
    const mySigHex = crypto.createHmac('sha256', signingSecret).update(basestring).digest('hex');
    const mySig = `v0=${mySigHex}`;

    const a = Buffer.from(mySig);
    const b = Buffer.from(sig);
    if (a.length !== b.length) {
      console.warn('verifySlackRequest: signature length mismatch');
      return false;
    }
    return crypto.timingSafeEqual(a, b);
  } catch (err) {
    console.error('verifySlackRequest error:', err && err.message ? err.message : err);
    return false;
  }
}
